...
# ---- (Optional) transform to relative abundance before Bray–Curtis ----
# species_mat <- decostand(species_mat, method = "total")
...
if ("Pair" %in% names(df2)) {
  perm <- adonis2(bray ~ Invasion_status, data = df2,
                  permutations = 999, strata = df2$Pair)
} else {
  perm <- adonis2(bray ~ Invasion_status, data = df2, permutations = 999)
}
print(perm)
